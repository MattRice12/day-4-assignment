...
class Game
  ...
  def move_computer
    ...
    oneone = @board.one
    twotwo = @board.two
    threethree = @board.three
    fourfour = @board.four
    fivefive = @board.five
    sixsix = @board.six
    sevenseven = @board.seven
    eighteight = @board.eight
    ninenine = @board.nine

    case computer_set.pop
    when "1" then computer_move_message(oneone, "1")
    when "2" then computer_move_message(twotwo, "2")
    when "3" then computer_move_message(threethree, "3")
    when "4" then computer_move_message(fourfour, "4")
    when "5" then computer_move_message(fivefive, "5")
    when "6" then computer_move_message(sixsix, "6")
    when "7" then computer_move_message(sevenseven, "7")
    when "8" then computer_move_message(eighteight, "8")
    when "9" then computer_move_message(ninenine, "9")
    end
  end

  def computer_move_message(v, n)
    v = "O"
    refresh_screen
    puts "Hal_9000 takes square #{n}"
    puts
  end
